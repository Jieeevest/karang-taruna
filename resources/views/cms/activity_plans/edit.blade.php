...
<div class="max-w-full mx-auto">
    <div class="flex items-center gap-4 mb-6">
        ...
        <div>
            <h2 class="text-3xl font-bold text-gray-800">Edit Rencana Kegiatan</h2>
            <p class="text-lg text-gray-600 mt-1">Perbarui informasi rencana kegiatan</p>
        </div>
    </div>
        ...
        <form action="{{ route('cms.activity-plans.update', $activityPlan) }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div>
                    ...
                    <select name="status" class="w-full text-base rounded-lg border-gray-300 focus:border-teal-500 focus:ring-teal-500 @error('status') border-red-500 @enderror" required>
            ...
            <div class="mb-6">
                ...
                <textarea name="objectives" rows="3" 
                    class="w-full text-base rounded-lg border-gray-300 focus:border-teal-500 focus:ring-teal-500 @error('objectives') border-red-500 @enderror">{{ old('objectives', $activityPlan->objectives) }}</textarea>
                @error('objectives')
                    <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div id="rejection_reason_container" class="mb-6 {{ old('status', $activityPlan->status) == 'rejected' ? '' : 'hidden' }}">
                <label class="block text-base font-medium text-red-700 mb-2">Alasan Penolakan</label>
                <textarea name="rejection_reason" rows="3" 
                    class="w-full text-base rounded-lg border-red-300 focus:border-red-500 focus:ring-red-500 bg-red-50"
