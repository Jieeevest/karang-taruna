    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <form action="{{ route('cms.categories.update', $category) }}" method="POST">
            ...
            <div class="mb-6 p-4 bg-blue-50 rounded-lg">
                <p class="text-sm text-blue-800 font-medium mb-2">Statistik Penggunaan:</p>
                <div class="flex items-center space-x-4 text-sm text-blue-700">
                    <span>{{ $category->activityPlans()->count() }} Kegiatan</span>
                    <span>•</span>
                    <span>{{ $category->contents()->count() }} Konten</span>
                </div>
            </div>
