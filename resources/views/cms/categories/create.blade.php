        ...
        <form action="{{ route('cms.categories.store') }}" method="POST">
            ...
            <div class="mb-6">
                ...
                <select name="type" id="type"
                    ...
                    <option value="activity" {{ old('type') == 'activity' ? 'selected' : '' }}>Kegiatan (Activity)</option>
                    <option value="document" {{ old('type') == 'document' ? 'selected' : '' }}>Dokumentasi (Document)</option>
                    <option value="content" {{ old('type') == 'content' ? 'selected' : '' }}>Konten (Content)</option>
                </select>
                @error('type')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
